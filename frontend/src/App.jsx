...
async function api(path, options) {
  ...
  try {
    ...
    throw new Error("Không thể kết nối API. Vui lòng thử lại sau.");
  }
    ...
    <main>
      <header className="hero">
        <div>
          ...
          <h1>StockLite</h1>
          <p>Theo dõi sản phẩm và số lượng tồn kho trong một nơi.</p>
        </div>
        <button className="secondary refresh" onClick={load}>
          ↻ Làm mới dữ liệu
        </button>
      </header>
      ...
      <section className="workspace">
        <form className="card product-form" onSubmit={submitProduct}>
          <div className="section-heading">
            <div>
              <p className="eyebrow">
                {editingId ? "CHỈNH SỬA" : "SẢN PHẨM MỚI"}
              </p>
          ...
          <Field label="Mã sản phẩm (SKU)" hint="Ví dụ: PEN-01">
            <input
              required
              value={form.sku}
              onChange={(event) =>
          ...
          <div className="number-fields">
            ...
            <Field label="Báo tồn thấp khi còn">
              <input
                required
                min="0"
                type="number"
        ...
        <aside className="card guide">
          <p className="eyebrow">CÁCH DÙNG NHANH</p>
          <h2>Ba bước đơn giản</h2>
          <ol>
            <li>
              <b>Thêm</b> sản phẩm vào danh sách.
            </li>
            <li>
              Nhấn <b>Nhập / xuất kho</b> khi số lượng thay đổi.
            </li>
            <li>
              Kiểm tra nhãn <b>Sắp hết</b> để nhập thêm hàng.
            </li>
          </ol>
        </aside>
      </section>
      ...
      <section className="card product-list">
        ...
        {loading ? (
          ...
          <div className="empty">
            ...
            <span>Hãy thêm sản phẩm đầu tiên ở biểu mẫu phía trên.</span>
          </div>
        ) : (
                    ...
                    <tr key={product.id}>
                      ...
                      <td>
                        <span className={low ? "badge low" : "badge"}>
                          {low ? "Sắp hết hàng" : "Đủ hàng"}
                        </span>
                      </td>
                      <td className="actions">
                        <button onClick={() => setMovement(product)}>
                          Nhập / xuất
                        </button>
                        <button
      ...
      {movement && (
        <div className="modal-backdrop">
          <form className="modal" onSubmit={submitMovement}>
            <button
              type="button"
              className="close"
            ...
            <p className="eyebrow">CẬP NHẬT TỒN KHO</p>
            <h2>{movement.name}</h2>
            <p className="current-stock">
              Đang có: <strong>{movement.quantity}</strong> sản phẩm
            </p>
            <Field label="Bạn muốn làm gì?">
              <select name="type">
                <option value="IN">Nhập thêm vào kho</option>
                <option value="OUT">Xuất hàng khỏi kho</option>
              </select>
            </Field>
            ...
            <Field label="Ghi chú (không bắt buộc)">
              <input name="note" placeholder="Ví dụ: Nhập từ nhà cung cấp" />
            </Field>
            <button className="primary-submit">Lưu cập nhật kho</button>
          </form>
        </div>
      )}
    </main>
